...
class Dashboard extends React.Component {
      ...
      if (res.data && res.data.submits.length > 0) {
        let events = res.data.submits.reduce((acc, cur) => {
          ...
          if (index !== -1) {
            ...
            acc[index].title = `+ ${acc[index].projects.length} Project${
              acc[index].projects.length > 1 ? "s" : ""
            }`;
          } else {
            acc.push({
  ...
  render() {
    const { team, openModal, dataOfModal } = this.state;
    return (
      <span>
        <Breadcrumb>
          <BreadcrumbItem>
            <Link to="/team">ĐỘI</Link>
          </BreadcrumbItem>
          {team.name && (
            <BreadcrumbItem active>{team.name.toUpperCase()}</BreadcrumbItem>
          )}
        </Breadcrumb>
        <h4
